...
func TestDB_ListKeys(t *testing.T) {
	...
	// 只有一条数据
	err = db.Put(utils.GetTestKey(11), utils.GetTestKey(20))
	assert.Nil(t, err)
	keys2 := db.ListKeys()
	assert.Equal(t, 1, len(keys2))

	// 有多条数据
	err = db.Put(utils.GetTestKey(22), utils.GetTestKey(20))
	assert.Nil(t, err)
	err = db.Put(utils.GetTestKey(33), utils.GetTestKey(20))
	assert.Nil(t, err)
	err = db.Put(utils.GetTestKey(44), utils.GetTestKey(20))
	assert.Nil(t, err)
