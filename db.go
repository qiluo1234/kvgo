...
func (db *DB) loadDataFiles() error {
	...
	var fileIds []int
	//遍历目录中所有文件，找到data结尾的数据文件
	for _, entry := range dirEntries {
		if strings.HasSuffix(entry.Name(), data.DataFilesNameSuffix) {
			splitNames := strings.Split(entry.Name(), ".")
			fileId, err := strconv.Atoi(splitNames[0])
			//数据目录有可能被损坏
			if err != nil {
				return ErrDataDirectryCorrupted
			}
